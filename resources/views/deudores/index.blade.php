                        ...
                        <div class="row">
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    ...
                                    <input type="date" id="fecha_contabilizacion" name="fecha_contabilizacion" class="form-control">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="numero_documento" name="numero_documento" class="form-control" style="display:none;">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="cuenta_bancaria">Cuenta Bancaria:</label>
                                    <select id="cuenta_bancaria" name="cuenta_bancaria" class="form-control" style="display:none;">
                                        <option value="0">Seleccione un cliente</option>
                                        @foreach($cuentasBancarias as $pago)
                                            <option value="{{ $pago->id }}">{{ $pago->Bancos->nombre }} - {{ $pago->nombre_cuenta }} - {{ $pago->numero_cuenta }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row mt-3">
                            <div class="col-md-3">
                                <div class="form-group row">
                                    <label for="saldo_total" class="col-md-4 col-form-label text-left">Saldo total:</label>
                                    <div class="col-md-8">
                                        <input type="text" id="saldo_total" name="saldo_total" class="form-control text-right" disabled>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="monto_seleccionado" class="col-md-4 col-form-label text-left">Monto seleccionado:</label>
                                    <div class="col-md-8">
                                        <input type="text" id="monto_seleccionado" name="monto_seleccionado" class="form-control text-right" disabled>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="monto_a_operar" class="col-md-4 col-form-label text-left">Monto a operar:</label>
                                    <div class="col-md-8">
                                        <input type="text" id="monto_a_operar" name="monto_a_operar" class="form-control text-right">
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            fetch(`/deudores/documentos/${clienteId}`)
                ...
                .then(data => {
                    ...
                    data.forEach(documento => {
                        ...
                        var acumuladoPagos = parseFloat(documento.totalAcumuladoPagos).toFixed(2);
                        ...
                        row.addEventListener('click', function() {
                            ...
                            fetch(`/deudores/pagos-aplicados/${documentoId}`)
                                .then(response => response.json())
                                .then(pagos => {
                                    var pagosAplicadosTable = document.getElementById('pagos-aplicados-table').querySelector('tbody');
                                    ...
                                    pagos.forEach(pago => {
                                        ...
                                        `;
                                        pagosAplicadosTable.appendChild(row);
                                    });
                                });
                        });
                    });
                    ...
                    document.querySelectorAll('.document-checkbox').forEach(function(checkbox) {
                        checkbox.addEventListener('change', function() {
                            ...
                            if (this.checked) {
                                documentosSeleccionados.push({ id: documentoId, saldo: saldo, fechaDocto: fechaDocto });
                            } else {
                                documentosSeleccionados = documentosSeleccionados.filter(doc => doc.id !== documentoId);
                            }

                            var montoSeleccionado = documentosSeleccionados.reduce((acc, doc) => acc + doc.saldo, 0);
                            document.getElementById('monto_seleccionado').value = montoSeleccionado.toFixed(2);

                            // Ordenar documentos seleccionados por fecha en orden ascendente
                            documentosSeleccionados.sort(function(a, b) {
                                return new Date(a.fechaDocto) - new Date(b.fechaDocto);
                            });
                        });
                    });
                });
