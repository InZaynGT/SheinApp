...
class ListadoDeudoresController extends Controller
{
    public function index()
    {
        ...
        $deudores = CxcDocumentoModel::selectRaw('clientes.nombre, COALESCE(SUM(cxcdocumento.saldoDocto), 0) as saldo_total, cxcdocumento.*')
            ->join('clientes','cxcdocumento.idCliente', '=', 'clientes.id')
            ->where('cxcdocumento.saldoDocto', '>',0)
            ->orderBy('saldo_total','DESC')
            ->groupBy('cxcdocumento.idCliente','clientes.nombre','cxcdocumento.id')
            ->get();
